...
It also contains several **environment files** (with suffix env.json) with preconfigured variables whose values you can update within Postman (by clicking on the cog icon near the top-right of Postman). We refer to the postman collection in the usage section below*
