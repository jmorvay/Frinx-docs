...
The easiest way to issue the REST calls detailed below is using Postman and our preconfigured REST calls packaged  - see our [usage guide](https://frinxio.github.io/Frinx-docs/FRINX_ODL_Distribution/Boron/API.html). Follow the guide to import the file `postman_collection_FRINX API Bundle.json` from within the directory `Infrastructure`.
