...
| CLI plugin:           |             |                                                                                                                                                                                                                                                                                |
| Feature introduced in | FRINX 3.1.0 | Safe-command-execution for CLI connections - the CLI session now waits for the device to echo back each command. So whenever a device takes longer to process a particular command, Frinx ODL waits before issuing the subsequent one.                                         |
| Feature introduced in | FRINX 3.1.0 | Reconciliation – CLI cache in Frinx ODL can now be also reconciled with 'read configuraton root' (in the case of RESTCONF it’s a GET operation to the entire configuration subtree of a mounted device).                                                                       |
